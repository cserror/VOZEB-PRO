...
- 当前阶段：官方 `v0.0.7` 基线上已完成首个业务二开切片（用户生成记录、提示词入口合并和结果操作），本地验证完成，尚未 push 或部署生产服务器。
...
- 当前切片通过 Web 全量 Vitest、类型检查、Lint、生产构建和目标浏览器回归；已纳入本地 Git 提交，尚未 push 或上线交接。
...
1. 首个业务二开切片完成本地提交后，由用户决定继续二开、push 或进入上线交接；不自动 push 或部署。
2. 准备首次部署时先形成明确 commit 和上线交接单，再由运维窗口执行完整审查；未经用户生产操作确认不得部署。
3. 审查时确定目标服务器、域名、端口、反向代理、证书、资源容量、生产环境变量、备份和回滚方案。
4. 生产部署后初始化独立数据库和管理员，再通过后台配置品牌与模型渠道；完成生产健康、登录、持久化和真实业务流程验收后才更新部署状态。
