...
- 当前阶段：法国封闭测试环境已部署 `13ee230`；本地在此基线上完成自定义异步协议“任务 ID 字段”配置，支持按真实字段路径读取文本、图片、视频和音频任务 ID，尚未推送或部署。
- 最近核验时间：2026-09-02，Asia/Shanghai。
- 核验范围：本地 Git、运维台账中的线上版本、任务 ID 字段的保存/解析/运行时读取、相关 Vitest、类型检查，以及后台渠道编辑器桌面与 390px 浏览器回归。
...
- 当前线上与 `origin/main` 基线为 `13ee2308ef8ce10b5d029fe99b68b78cc53c49a2`；本次任务 ID 字段改造将在后继本地 `main` commit 中交接，远端在明确 push 前仍不包含该能力。
...
- 生成记录、提示词入口与 R2/CDN 媒体改造均已部署到法国封闭测试环境的 `13ee230`；本次任务 ID 字段改造尚未部署。
...
2. 运维窗口需按交接单核验法国封闭测试服务的实际版本和回滚点；本次不应修改数据库、环境变量、Compose、端口、域名或持久化数据。
3. 生产操作需单独确认；部署后先验证健康、Web/Worker 版本和后台字段回读，再使用有限额度完成一次 AutoDL 创建、轮询、成片与扣费验收，由运维台账更新实际部署版本。
