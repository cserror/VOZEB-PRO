...
- 2026-09-09：本隔离工作树 HEAD 为已批准并推送的依赖修补 `2b671c325e46c5f71375207acb10638428325b62`，业务来源为 `61a5c2f6bee5fa6c4e66f662d143ea01bbab2e65`。历史两次云端 Quality 失败，尚无成功 Linux 镜像；远端实时状态需推送前再核。
- 用户已批准按测试阶段精简检查。本地流程候选默认只做密钥扫描、Docker 内类型/编译和成品隔离验证；全量质量检查改为手动选择。当前配置、发布清单及针对性测试已调整，未提交、推送、触发新云构建或部署。此前未提交的 21 文件格式调整、E2E 修补和未成功的手机布局实验保留在工作树，但不纳入本次候选。
...
- 本轮发布清单/隔离安全测试 26 项、工作流契约测试 6 项和 Actionlint 通过；未重跑应用构建、手机页面或全量套件。真实 Linux 镜像构建/成品测试、Actions job 名称与报告字段映射、GHCR 读取及生产验收仍未执行。部署方式尚未迁移，以下业务与运行描述为历史记录，不代表本轮新验收。
...
- 核验范围：本地 Git、视频协议预设、保存/同步校验、创建请求及隔离后台，以及 `server-admin/projects/vozeb-pro.md`。线上版本引用运维台账，部署前须重新核验现场。下方本地运行及全量验证条目为历史证据，不代表本轮重新执行。

### 2026-09-07 本地 New API 多模态参考修复

- 根据已确认的插件契约，普通参考图、视频和音频改为有序 URL 数组，显式首帧独立发送；不新增固定数量上限，后台模型能力约束不变。契约 owner 为 `docs/content/docs/backend/api-response.mdx`。
- 11 份局部 Vitest 共 203 项通过，包含本地 TCP 的 0/1/2/8/16 张图、独立首帧、混合媒体、480p/720p、后台保存/回读/同步及既有数量约束；TypeScript 通过，局部 ESLint 0 错误、3 条既有警告。未执行真实上游、付费生成或完整生产构建。
- 独立文件 Provider 与虚构凭据下，真实浏览器验证渠道创建、模型级预设、保存及刷新后回读；1440/390/430 宽度下模板与三类参考能力保留，输入框无横向溢出。截图位于本地未跟踪证据目录 `output/playwright/newapi-multimodal-20260907/`，不作为源码发布。文件 Provider 的通知接口返回既有 409，另有 Ant Design 开发警告，未影响本次配置流程；未覆盖真实 Canvas 成片及深色主题矩阵。本轮隔离服务验收后关闭，不改变常用运行配置。
- 只修改本地源码、测试和契约；上线需另行更新目标渠道的视频模型级/操作级模板与参考能力，不自动修改历史任务、账务或真实渠道配置。

### 已有基线：New API 单图 JSON URL 修复（dfe41b8）

- 该基线将 New API 视频预设与后台严格保存/同步规则统一为 JSON `input_reference` URL，显式首帧同样映射到该字段；此单图契约已被上方本地多模态修复替代。现行接口契约见 `docs/content/docs/backend/api-response.mdx`。
- 实现阶段记录：10 个相关 Vitest 文件共 186 项通过，覆盖 480p/720p 普通参考与首帧的真实本地 TCP 请求，以及保存、回读、严格模型同步和再次保存；TypeScript 通过，局部 ESLint 0 错误、6 条既有警告。独立文件 Provider、虚构凭据和不可调用的上游地址下，Chrome 后台保存并刷新后仍显示 JSON URL。未使用真实用户数据或付费模型，未覆盖完整移动端/主题矩阵或真实成片。
- 提交交接复核：2026-09-07 重跑协议注册表、后台设置 Route、active-protocol-media-matrix 三份测试，78 项通过；TypeScript 通过，五个相关 TS 文件 ESLint 0 错误、6 条警告。未重跑实现阶段浏览器验收或完整构建，未连接服务器。原有开发生成文件和旧插件不纳入提交，使用本次 commit 的独立干净工作树和 `git archive` 交接。
- 上线需同时更新代码和目标渠道已保存的视频模型级/操作级模板；不会自动迁移数据库、历史任务或账务。当前工作区既有 `web/next-env.d.ts` 和 `new-api-binghuo/` 改动仍不属于本轮。

### 已有基线：视频结果兼容修改（04c1c6d）
...
- 该基线只发布 VOZEB 代码，不包含 New API 插件升级；运维台账在部署审查时记录实际激活插件为 `video-aggregator v0.2.6`。生产状态与回滚点继续由运维台账拥有。
...
- 该基线已由运维记录部署，但真实成片验收仍未关闭；已有失败且已退款任务另行确认恢复，不重新创建视频。源码预设不会批量覆盖已保存的渠道配置或任务快照。
...
- 运维台账记录生成记录、提示词入口与 R2/CDN 媒体改造已于 `13ee230` 部署，任务 ID 字段改造已于 `9fcb1aa` 部署，视频结果获取兼容已于 `04c1c6d` 部署，单图 JSON URL 预设已于 `dfe41b8` 部署；本轮多模态参考修复尚未部署。
