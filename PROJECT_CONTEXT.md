...
- 当前阶段：官方 `v0.0.7` 基线上已完成首个业务二开切片（用户生成记录、提示词入口合并和结果操作），本地验证完成并已推送到 `origin/main`；法国封闭测试环境仍运行旧版 `0a247aa`，本次更新尚未部署。
- 最近核验时间：2026-08-25，Asia/Shanghai。
- 核验范围：本地 Git、依赖与构建、PostgreSQL 结果分页、用户生成记录、提示词入口、桌面/移动端页面回归，以及此前完成的安装向导和管理员后台基础验收。
...
- 当前切片通过 Web 全量 Vitest、类型检查、Lint、生产构建和目标浏览器回归；已纳入 Git 提交并推送到 `origin/main`，尚未完成上线审查或部署。
...
1. 以 `origin/main` 的明确 commit 生成上线交接单，由运维窗口审查法国封闭测试服务从 `0a247aa` 更新到本次版本；未经用户生产操作确认不得部署。
2. 审查时重新核验目标服务器、域名、端口、反向代理、证书、资源容量、生产环境变量、备份和回滚方案，并确认本次无需数据库迁移或配置变更。
3. 部署后验证健康、登录、生成记录、提示词入口、历史数据兼容和 Worker 状态，再由运维台账更新实际部署版本；不得把封闭测试更新表述为正式商业上线。
