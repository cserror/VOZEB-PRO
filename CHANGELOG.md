...
- [视频渠道] New API 视频改为聚合插件多模态 URL 模板，按原顺序完整传递普通参考图、视频和音频，首帧独立发送，空参考字段省略；数量限制沿用后台配置。已有渠道模板与参考能力需上线时另行更新，历史任务不变。
- [视频渠道] New API 视频预设改为 JSON URL，使后台保存校验和模型同步使用同一模板；补齐显式首帧到 `input_reference` 的映射。已有渠道模板需上线时另行更新，其他协议保持不变。
- [生成记录] 新增 `/generations` 用户页面和 `/api/generation-history`：图片、视频按单个结果每页 24 条分页，支持日期分组、筛选、详情、预览、复制提示词、继续修改、下载、幂等添加素材及当前页批量下载/删除；内部执行提示词不对外展示，结果删除继续保护已有媒体引用。
- [提示词] 将公共提示词库与我的提示词收敛到 `/prompts` 的两个视图，导航只保留“提示词”，旧 `/my-prompts` 自动重定向到个人提示词视图。
- [构建] Standalone 复制 Sharp 运行包时保留 pnpm 相对符号链接，避免重复准备产物在 Node.js 24 下因目标链接指回源码而报 `ERR_FS_CP_EINVAL`。
