...
import { getLocalMediaRegistration, isLocalMediaPendingDeletion, isLocalMediaRegistrationExpired, type LocalMediaRegistration } from "@/lib/server/local-media-registry";
...
async function serveReferenceAsset(request: Request, context: RouteContext) {
    ...
    if (signature && (!registration || isLocalMediaRegistrationExpired(registration) || isLocalMediaPendingDeletion(registration))) return NextResponse.json({ error: "媒体文件不存在或已过期" }, { status: 404 });
    ...
    if (isLocalMediaRegistrationExpired(registration) || isLocalMediaPendingDeletion(registration)) return NextResponse.json({ error: "媒体文件不存在或已过期" }, { status: 404 });
