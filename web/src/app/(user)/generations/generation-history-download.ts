...
export async function downloadGenerationHistoryZip(items: GenerationHistoryItem[], signal?: AbortSignal) {
    const downloadable = items.filter((item): item is GenerationHistoryItem & { asset: NonNullable<GenerationHistoryItem["asset"]> } => item.status === "success" && Boolean(item.asset?.storageKey));
    if (!downloadable.length) throw new Error("所选记录中没有可下载的结果");
    let totalBytes = 0;
    const files = async function* () {
        for (const [index, item] of downloadable.entries()) {
            const operationUrl = mediaRouteUrl("generation", item.asset.storageKey);
            const source = item.kind === "image" ? originalImageDownloadUrl(operationUrl) : originalMediaDownloadUrl(operationUrl);
            ...
            const media: AgentMediaDownload = { type: item.kind, url: operationUrl, title: item.title, mimeType: item.asset.mimeType };
