...
export async function createDramaProject(userId: string, project: DramaProject) {
    return withDramaMediaReferenceWrite(userId, collectLocalMediaStorageKeys(project), async (executor) => {
        if (getDatabaseProvider() === "postgres") {
            await ensurePostgresSchema();
...
export async function updateDramaProject(userId: string, project: DramaProject, expectedUpdatedAt?: string) {
    return withDramaMediaReferenceWrite(userId, collectLocalMediaStorageKeys(project), async (executor) => {
        if (getDatabaseProvider() === "postgres") {
            ...
            if (!result.rows[0]) {
                const existing = await query<{ id: string }>("SELECT id FROM drama_projects WHERE id = $1 AND user_id = $2", [project.id, userId]);
...
async function withDramaMediaReferenceWrite<T>(userId: string, storageKeys: string[], write: (executor?: QueryExecutor) => Promise<T>) {
    try {
        return await withActiveMediaReferenceWrite(storageKeys, { ownerUserId: userId }, write);
    } catch (error) {
        if (error instanceof MediaReferenceWriteConflict) throw new DramaProjectStoreError(error.message, error.status);
        throw error;
    }
}
