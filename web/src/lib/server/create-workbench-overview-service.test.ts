...
describe("create workbench overview service", () => {
    it("returns four running tasks and eight latest unique stable assets", () => {
        const logs = [
            ...
            generationLog("success-new", "success", "2026-07-26T12:00:00.000Z", [{ type: "image", storageKey: "permanent/image-one.webp" }]),
            generationLog("success-old", "success", "2026-07-25T12:00:00.000Z", [
                { type: "image", storageKey: "permanent/image-one.webp" },
                ...Array.from({ length: 9 }, (_, index) => ({ type: "video" as const, storageKey: `permanent/video-${index}.mp4` })),
            ]),
        ];
        ...
        expect(overview.recentAssets[0]).toMatchObject({ id: "success-new-0", storageKey: "permanent/image-one.webp", displayUrl: "/api/generation-log-assets/permanent/image-one.webp" });
        expect(overview.recentAssets.filter((asset) => asset.storageKey === "permanent/image-one.webp")).toHaveLength(1);
    });
